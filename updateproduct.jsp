    ...
    <script>
        function success() 
        {
            swal ({
                ...
                window.location="userpendingproduct.jsp";
            });
        }
            ...
            if ((contentType.indexOf("multipart/form-data") >= 0)) 
            {
               ...
               { 
                  ...
                  { 
                    ...
                    PreparedStatement st = con.prepareStatement("update product set prd_name='" + prdname + "',cat_id='" + category + "',seller_name='" + sellername + "',min_bid='" + minbid + "',bid_date='" + biddate + "',bid_time='" + bidtime + "',description='" + description + "',prd_img='" + fname + "' where prd_id='" + session.getAttribute("pid").toString() + "'");
                    st.executeUpdate();
                    st.close();
                    con.close();
                    %> 
            ...
            <%
                if(session.getAttribute("pid")!=null)
                {
                    try{
                        ...
                        ResultSet rs = st.executeQuery("select * from product where prd_id='" + session.getAttribute("pid").toString() + "'");
                        while(rs.next())
                        { %>
                        
                            <div class="col-md-12 form-group">
                                <input type="text" class="form-control" name="txtprdname" placeholder="Product Name" value="<%= rs.getString(2) %>" required>
                            </div>
                            ...
                            %>
                                <select class="country_select" id="ddcategory" name="ddcategory">
                            ...
                            <% } catch(Exception e){} 
                                ...
                                <script>
                                    document.getElementById("ddcategory").selectedIndex=<%= rs.getInt(3) %>-1;
                                </script>
                            </div>
                            
                            <div class="col-md-12 form-group">
                                <input type="number" class="form-control" name="txtminbid" placeholder="Minimum Bid" value="<%= rs.getInt(5) %>" required>
                            </div>
                            ...
                            <div class="col-md-6 form-group">
                                <input type="date" class="form-control" name="txtbiddate" value="<%= rs.getString(6) %>" required>
                            </div>
                            ...
                            <div class="col-md-6 form-group">
                                <input type="time" class="form-control" name="txtbidtime" value="<%= rs.getString(7) %>" required>
                            </div>
                            
                            <div class="col-md-12 form-group">
                                <textarea class="form-control" name="txtdes" rows="1" required><%= rs.getString(8) %></textarea>
                            </div>
                            ...
                            <div class="col-md-6 form-group">
                             ...
                             <div class="col-md-6 form-group" style="margin-left: auto;margin-right: auto;margin-top: 30px;">
                                <button type="submit" value="submit" class="button button-login w-100">Update the Product</button>
                            </div>
                        <% }
                    }catch(Exception e){} finally{con.close();}
                }
